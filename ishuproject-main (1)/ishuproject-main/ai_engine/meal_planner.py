...
class MealPlanner:
    ...
    def _calculate_daily_nutrition(self, day_meals: Dict) -> Dict:
        ...
        from models.food import FoodItem
        from ai_engine.nutrition_estimator import NutritionEstimator
        ...
        for meal_type, foods in day_meals.items():
            for food_data in foods:
                ...
                if food:
                    # Use nutrition estimator for accurate values
                    nutrition = NutritionEstimator.get_nutrition_with_estimate(food)
                    
                    try:
                        total_nutrition['calories'] += float(nutrition.get('calories', 0) or 0)
                        total_nutrition['protein'] += float(nutrition.get('protein', 0) or 0)
                        total_nutrition['iron'] += float(nutrition.get('iron', 0) or 0)
                        total_nutrition['calcium'] += float(nutrition.get('calcium', 0) or 0)
                        total_nutrition['fiber'] += float(nutrition.get('fiber', 0) or 0)
                        total_nutrition['folic_acid'] += float(nutrition.get('folic_acid', 0) or 0)
            ...
            if daily_nutrition:
                for nutrient in totals.keys():
                    ...
                    try:
                        ...
                        if numeric_value > 0:
                            totals[nutrient] += numeric_value
                    except (ValueError, TypeError):
                        pass
                valid_days += 1
        
        # If no nutrition data, return formatted defaults
        if valid_days == 0 or all(v == 0 for v in totals.values()):
            return {
                'total_days': total_days,
                'avg_daily_calories': '2000-2500 kcal',
                'avg_daily_protein': '60-70g',
                'avg_daily_iron': '27mg',
                'avg_daily_calcium': '1000mg',
                'avg_daily_fiber': '25-30g',
                'avg_daily_folic_acid': '600mcg',
                'note': 'Recommended values for pregnancy'
            }
        
        # Safely calculate averages
        avg_days = max(valid_days, 1)
        
        # Format with proper units
        return {
            'total_days': total_days,
            'avg_daily_calories': f"{int(totals['calories'] / avg_days)} kcal" if totals['calories'] > 0 else '2200 kcal',
            'avg_daily_protein': f"{totals['protein'] / avg_days:.1f}g" if totals['protein'] > 0 else '60g',
            'avg_daily_iron': f"{totals['iron'] / avg_days:.1f}mg" if totals['iron'] > 0 else '27mg',
            'avg_daily_calcium': f"{totals['calcium'] / avg_days:.1f}mg" if totals['calcium'] > 0 else '1000mg',
            'avg_daily_fiber': f"{totals['fiber'] / avg_days:.1f}g" if totals['fiber'] > 0 else '25g',
            'avg_daily_folic_acid': f"{totals['folic_acid'] / avg_days:.1f}mcg" if totals['folic_acid'] > 0 else '600mcg'
        }
